						...
						<div class="card card-shadow mb-4">
							<div class="table-responsive">
								<table class="table table-sm table-bordered">
									<thead>
										<tr>
											<th><label class="control control-solid control-solid-info control--checkbox mb-4" style="display:inline;"><input type="checkbox" id="check_all"><span class="control__indicator"></span></label><button class="btn btn-info btn-sm" onclick="download_running('download')"><i class="fa fa-download"></i></button><button class="btn btn-info btn-sm" onclick="download_running('mark_paid')">MP</button></th>
											<th>Order #</th>
											<th>Customer</th>
											<th>Cost Centre</th>
											<th>Company</th>
									...
									<tbody>
										<?php if(!empty($orders)){
											...
											{
												echo "<tr>";
												echo "<td><label class=\"control control-solid control-solid-info control--checkbox\"><input type=\"checkbox\" class=\"running_sheet_check\" id=\"order_".$order->order_id."\"><span class=\"control__indicator\"></span></label></td>";
									...
									echo "<button class=\"btn btn-danger btn-sm ml-1\" onclick=\"delete_order('".$order->order_id."','".$referrer."')\">Cancel</button>";
												...
												if($order->order_status==15 || $order->order_status==99){
...
$(function(){
    ...
    var options = customer.getElementsByTagName("option");
    var optionHTML = options[customer.selectedIndex].innerHTML;
    
   var firstn = optionHTML.split(" ")[0];
    var lasstn = optionHTML.split(" ")[1];
 if(lasstn == ''){
     var lasstn = optionHTML.split(" ")[2];
 }
 
  if(lasstn == ''){
     var lasstn = optionHTML.split(" ")[3];
 }
...
if(lasstn=='Customers'){
    lasstn = 'unset';
		...
		if(window.location.href.indexOf('order_history')!=-1)
			window.location.href="<?php echo base_url();?>index.php/orders/order_history/"+date_from+"/"+date_to+"/"+$("#company").val()+"/"+firstn+"/"+lasstn+"/"+sort_order+"/myy/"+order_id;
		else
			window.location.href="<?php echo base_url();?>index.php/orders/order_history/"+date_from+"/"+date_to+"/"+$("#company").val()+"/"+firstn+"/"+lasstn+"/"+sort_order+"/myob"+"/"+order_id;
	})
   ...
   comment = $("#group_mark_paid_comments").val();
    var running_sheet=[];
      ...
      $(".running_sheet_check").each(function(){
       ...
       window.open('<?php echo base_url();?>index.php/orders/group_mark_as_paid/'+running_sheet+'/'+comment+"/"+pathh);
